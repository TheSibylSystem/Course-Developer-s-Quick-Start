          ...
          if (links[index].id.indexOf('live') + 1) {
            window.location.replace(
              location.protocol +
                ...
                location.search +
                '#' +
                links[index].id.replace('live-', 'live')
            );
          }

          LightBox.find('.gallery-description-body .fa')
            .removeClass()
            .addClass(data[index]['icon']);
...
$(function () {
  ...
  var Modifier = function () {
    ...
    var Navbar = function () {
      ...
      var toggle = function () {
        if (
          element.find('.container, .container-fluid').outerWidth() <
          element.find('.navbar-nav').outerWidth()
        ) {
          element
            .find('.container')
        ...
        element
          ...
          .css({ 'max-height': getWindowSize().height - 51 });
      };
      toggle();
  ...
  var initPortfolio = function () {
    ...
    var getData = function () {
      ...
      wrapper.find('[data-blueimp]').each(function () {
        temp.push({
          icon:
            typeof $(this).attr('data-icon') != 'undefined'
              ? $(this).attr('data-icon')
              : 'fa fa-image',
          description:
            typeof $(this).attr('title') != 'undefined'
              ...
              : '',
          date:
            typeof $(this).attr('data-date') != 'undefined'
              ? $(this).attr('data-date')
              : '',
          soc: '', // Alexeev
        });
    ...
    if (slider.length) $(window).on('load.initPortfolio', startSlider);
  };
  ...
  var initSliderResponsive = function () {
    ...
    var getMinMarge = function (newItems) {
      ...
      try {
        var center =
          newItems.eq(0).outerWidth(true) + newItems.eq(1).outerWidth(true) / 2;
    ...
    var calculateHeight = function () {
      slider
        .parent()
        .height(slider.find('.slider-responsive-inner-item').height());
      slider.height(slider.find('.slider-responsive-inner-item').height());
      if (long && !isMin) {
        slider.parent().css({
          marginLeft: getMinMarge(
            $('.slider-responsive-inner-item').slice(0, 2)
          ),
        });
      }
    };
            ...
            onBefore: function (data) {
              $(this)
                ...
                .animate(
                  {
                    marginLeft:
                      long && !isMin ? getMinMarge(data.items.visible) : 0,
                  },
                  data.scroll.duration
                );
              data.items.old
                .eq(+long)
                .animate(defaultItem, data.scroll.duration)
                .removeClass('active');
              data.items.visible
                .eq(+long)
                .animate(selectedItem, data.scroll.duration)
                .addClass('active');
            },
          },
          onCreate: function (data) {
            ...
            slider
              ...
              .height(slider.find('.slider-responsive-inner-item').height());
            slider.height(
              slider.find('.slider-responsive-inner-item').height()
            );
          },
        });
    };
  ...
  var Activities = function () {
    var activities = $('.activities-description .tab-content').closest(
