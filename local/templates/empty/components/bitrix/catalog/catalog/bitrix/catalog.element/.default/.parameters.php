...
// Если передан ID инфоблока, проверяем, не является ли он offer'ами для какого-то товара
$arSKU = false;
$boolSKU = false;
...
// Проверяет, есть ли физическая папка `themes` с CSS-файлами для каждой темы
$arThemes = array();
if (ModuleManager::isModuleInstalled('bitrix.eshop')) {
    $arThemes['site'] = GetMessage('CP_BCE_TPL_THEME_SITE');
}

// Код, который сканирует папку /themes/ и добавляет темы в список
$arThemesList = array(
...
// Этот блок выполнится только если в настройках уже выбран инфоблок
if (isset($arCurrentValues['IBLOCK_ID']) && 0 < intval($arCurrentValues['IBLOCK_ID'])) {
    // Создаются разные списки свойств по их типу
    $arAllPropList = array();
    ...
    // Запрос к БД для получения всех активных свойств инфоблока
    ...
    // Используя сформированные выше списки, создаются параметры
    ...
    // Если инфоблок является торговыми предложениями ($boolSKU = true), то добавляются дополнительные параметры
    if ($boolSKU) {
        ...
        while ($arProp = $rsProps->Fetch()) {
            ...
            if (
                ...
                || ('S' == $arProp['PROPERTY_TYPE'] && 'directory' == $arProp['USER_TYPE'] && CIBlockPriceTools::checkPropDirectory(
                        $arProp
                    ))
            ) {
...
// Параметры для вкл/выкл комментариев через Блоги, VK, Facebook
...
// 7. НАСТРОЙКИ ДЛЯ HIGHLOAD-БЛОКОВ (например, бренды)
